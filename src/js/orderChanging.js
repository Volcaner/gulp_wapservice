	...
	if(data.trade_list && data.trade_list.length>0){
		...
		for(var i in list){
			...
			for(var j in mylist){
				...
				var retOrderState = "";
				if(mylist[j].retOrderState == "Y" || mylist[j].retOrderState == "S" || mylist[j].retOrderState == "E" || mylist[j].retOrderState == "F"){
					retOrderState = "退货中";
				...
				cstr+=[
						'<section class="or_goods">',
							'<a href="/yich/wapservice/dist/html/goods_detail_page.html?proId='+mylist[j].shopInv.pro_id+'"><img src="'+_src+'"></a>',
							'<p>',
