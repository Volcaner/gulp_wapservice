...
var setOrdersData=function(data,flag){
	...
	if(data.trade_list && data.trade_list.length>0){
		...
		for(var i in list){
			...
			var buttonStr = '';
			if(list[i].alipay_num && list[i].alipay_num != "N" && list[i].tra_state == "120"){
				buttonStr = '<input type="button" class="click-btn-redcolor singelpayBtn" value="付款">';
			}
			...
			fstr = [
					'<li traId="'+list[i].tra_id+'" payNum="'+list[i].alipay_num+'" traState="'+list[i].tra_state+'" retState="'+list[i].return_state+'">',
						...
						'</p>',
						'<p><span>实付金额:<i>￥'+trueMoney+'</i></span>',
							'<span class="or_btnbox">',
