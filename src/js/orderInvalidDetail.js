...
$(function(){
	...
	$(document).on('click','.resetBtn',function(){
		...
		chekMTfunc(str,function(){
			$('.goodslist').find('section').each(function(){
...
function setOrdersDetailData(data){
	...
	for(var i in list){
		...
		cstr +=[
				'<section orderId="'+list[i].orderId+'" goodNo="'+list[i].shopInv.good_no+'">',
	...
	state.traId = data.tra_id;
	state.payNum = data.alipay_num;
	//订单编号
...
function cancelDelete(){
	$.ajax({
         ...
         success:function(data){
        	 ...
        	 if(data.result == "1"){
        		 window.location.replace("/yich/wapservice/dist/html/orderInvalid.html");
        	 }else{
    			 tisp.init({
 					text:"删除订单失败!",
 				})
