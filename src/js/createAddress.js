...
$(function(){
	...
	$('.landline').on("blur",function(){
		var _this = $(this);
		var prompt = new promptFunc();
		if($(this).val()&&!(/^(\d{3,4}-)?\d{5,8}(-\d{1,16})?$/.test($(this).val()))){
		    prompt.init({
		      type:"",
		      text:"座机格式错误",
		      /*func:function(){
	...
	$('#saveAdressBtn').click(function(){
		var prompt = new promptFunc()
		if($('.landline').val()&&!(/^(\d{3,4}-)?\d{5,8}(-\d{1,16})?$/.test($('.landline').val()))){
			prompt.init({
				type:"",
				text:"座机格式错误",
			});
					...
					success:function(data){
						...
						if(data.i > 0){
							tsp.init({
								text:'保存成功!',
								func: function(){
									 window.location.replace("/yich/wapservice/dist/html/addressAdmin.html");
								}
							})
						}
